      ...
      <div className="container pt-10">
        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-8 pb-26 items-start">
          <div className="border-r border-gray-100 pr-4 md:sticky md:top-24 max-h-[calc(100vh-120px)] overflow-y-auto">
            <h2 className="font-bold leading-5 text-black mb-4 font-poppins">Shop by Category</h2>
              ...
              {uniqueCategories.map((category) => (
                <li key={category} className="w-full">
                  <Link
                    ...
                    className={`capitalize block w-full text-left px-3 py-1.5 rounded-lg transition-all font-poppins ${selectedCategory === category
          ...
          <div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
              {filteredProducts.length > 0 ? (
                ...
                <p className="col-span-full text-center py-20 flex items-center justify-center mt-9 bg-[#eb6e1b] text-white text-4xl rounded-2xl">
                  Stoke Out
                </p>
              )}
            </div>
